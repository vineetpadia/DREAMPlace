...
template <typename DetailedPlaceDBType, typename StateType>
inline __device__ typename DetailedPlaceDBType::type compute_reorder_cost(
    DetailedPlaceDBType db, StateType state, int group_id, int inst_id,
    int permute_id, int offset) {
  using T = typename DetailedPlaceDBType::type;
...
template <
    ...
    typename StateType>
__global__ void compute_reorder_hpwl_reduce_and_apply(
    DetailedPlaceDBType db, StateType state, int group_id,
    int group_size, int offset) {
...
template <typename T>
inline __device__ void unique_instance_nets_warp(
    KReorderState<T> state, int inst_id, int lane_id,
  ...
  for (int inst_id = warp_id; inst_id < group_size; inst_id += num_warps) {
    ...
    for (int j = 0; j < K; ++j) {
      ...
      for (int node2pin_id = node2pin_id_bgn;
        ...
        if (selected) {
          unsigned lower_lanes = (1U << lane_id) - 1U;
          int output_id =
              instance_nets_size + __popc(selected_mask & lower_lanes);
          if (output_id < MAX_NUM_NETS_PER_INSTANCE) {
            auto& instance_net = instance_nets[output_id];
            instance_net.net_id = net_id;
            instance_net.node_marker = (1 << node_marker);
            instance_net.pin_offset_x[node_marker] =
                db.pin_offset_x[node_pin_id];
          }
        }
        instance_nets_size =
            min(MAX_NUM_NETS_PER_INSTANCE,
                instance_nets_size + __popc(selected_mask));
    ...
    __syncwarp(kFullWarpMask);
    unique_instance_nets_warp(
        state, inst_id, lane_id, instance_nets_size);
    if (lane_id == 0) {
